...
while Shuffling_Iterations < DesiredShuffles:
    ...
    plotTrajectoriesComparison(shuffledMap, rSS, ClosedLoopDataShuffledPID.x_glob, 100,dl_path)
    raw_input("Does this work?")
    # ========================================= 0a. Run the RSS-LMPC controller on new track =====================================
    ClosedLoopShuffledLMPC = ClosedLoopData(dt, TimeLMPC, v0)
    ShuffledLMPCOpenLoopData = LMPCprediction(N, n, d, TimeLMPC, numSS_Points, Laps) #to store open-loop prediction and safe sets
    ShuffledLMPCSimulator = Simulator(shuffledMap, 1, 1) #now this simulator only runs for one lap, with the LMPC flag ON
    ShuffledLMPCController = ControllerLMPC(numSS_Points, numSS_it, N, Qslack, Q_LMPC, R_LMPC, dR_LMPC, n, d, shift, dt, shuffledMap, Laps, TimeLMPC, LMPC_Solver)
    
    ShuffledLMPCController.addReachableSet(rSS,ruSS,rQfun,shuffledMap)
    ShuffledLMPCController.addTrajectory(ClosedLoopDataShuffledPID)
    
    x0           = np.zeros((1,n))
    x0_glob      = np.zeros((1,n))
    x0[0,:]      = ClosedLoopShuffledLMPC.x[0,:]
    x0_glob[0,:] = ClosedLoopShuffledLMPC.x_glob[0,:]
    
         
    for it in range(2, Laps):            
        ClosedLoopShuffledLMPC.updateInitialConditions(x0, x0_glob)       
        ShuffledLMPCSimulator.Sim(ClosedLoopShuffledLMPC, ShuffledLMPCController, ShuffledLMPCOpenLoopData) #this runs one lap at a time due to initialization!           
        if ShuffledLMPCController.feasible == 0:
            break
        else:
            ShuffledLMPCController.addTrajectory(ClosedLoopShuffledLMPC)
            onlyLMPController.addTrajectoryToSS(onlyLMPController.shuffledSS, onlyLMPController.shuffleduSS, onlyLMPController.shuffledQfun, ClosedLoopShuffledLMPC)
                # Reset Initial Conditions
            x0[0,:]      = ClosedLoopShuffledLMPC.x[0,:]
            x0_glob[0,:] = ClosedLoopShuffledLMPC.x_glob[0,:]
